...
const logFile = path.join("./bot/data", "log.txt");
...
const _rotateLog = () => {
    return new Promise((resolve, reject) => {
        if (fs.existsSync(logFile)) {
            const date = new Date();
            // get the new path
            const newFile = path.join("./bot/data", `log_${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}_${date.getHours()}-${date.getMinutes()}-${date.getSeconds()}.txt`);
            // print file rotation
